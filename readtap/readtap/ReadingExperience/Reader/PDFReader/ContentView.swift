...
struct ReaderView: View {
    ...
    -> WordSelection?
  {
    ...
    let sentence: String = {
      // Try full-sentence extraction using page.string + NLTokenizer
      let pageHeight = page.bounds(for: .mediaBox).height
      let posHint: CGFloat? = pageHeight > 0 ? (1.0 - (pointOnPage.y / pageHeight)) : nil
      if let fullSentence = Self.extractSentenceAroundWord(
        pageText: page.string,
        selectedWord: effectiveText,
        positionHint: posHint,
        rectOnPage: finalRectOnPage,
        page: page
      ) {
        #if DEBUG
        print("[SentenceExtract] NLTokenizer OK wordLen=\(effectiveText.count) sentenceLen=\(fullSentence.count)")
        #endif
        return fullSentence
      }
      // Fallback: line-level extraction
      if let lineSelection = page.selectionForLine(at: pointOnPage),
        let rawLine = lineSelection.string
      {
        ...
        #if DEBUG
        print("[SentenceExtract] FALLBACK line-level wordLen=\(effectiveText.count) lineLen=\(bounded.count)")
        #endif
        return bounded
      }
      #if DEBUG
      print("[SentenceExtract] NO CONTEXT wordLen=\(effectiveText.count)")
      #endif
...
extension ReaderView {
  ...
  // MARK: - Script-based sentence filtering

  private enum DominantScript { case latin, hangul, cjk, japanese, other }

  private static func dominantScript(of text: String) -> DominantScript {
    var latin = 0, hangul = 0, cjk = 0, kana = 0
    for scalar in text.unicodeScalars {
      let v = scalar.value
      // Latin: Basic + Extended (covers EN, FR, ES, DE, PT, etc.)
      ...
      // CJK Unified Ideographs (shared by Chinese/Japanese/Korean)
                  ...
                  || (0xF900...0xFAFF).contains(v) || (0x20000...0x2A6DF).contains(v) {
        cjk += 1
      }
    }
    // Japanese = kana present (may also have CJK kanji)
    if kana > 0 && kana + cjk >= latin && kana + cjk >= hangul { return .japanese }
    let maxCount = max(latin, hangul, cjk)
    if maxCount == 0 { return .other }
    if latin == maxCount { return .latin }
    if hangul == maxCount { return .hangul }
    return .cjk
  }

  /// Returns true if a token is contextless noise (standalone numbers, codes like "26005-0013").
  private static func isNoiseToken(_ token: String) -> Bool {
    let stripped = token.trimmingCharacters(in: .punctuationCharacters.union(.symbols))
    if stripped.isEmpty { return false }
    return stripped.allSatisfy { $0.isNumber || $0 == "-" || $0 == "." }
  }

  /// Check if two scripts are compatible (should be kept together in a sentence).
  /// CJK kanji and Japanese kana are compatible since Japanese uses both.
  private static func scriptsCompatible(_ a: DominantScript, _ b: DominantScript) -> Bool {
    if a == b { return true }
    // Japanese text mixes kana and CJK kanji
    if (a == .japanese && b == .cjk) || (a == .cjk && b == .japanese) { return true }
    return false
  }

  private static func tokenMatchesScript(_ token: String, _ target: DominantScript) -> Bool {
    let letters = token.unicodeScalars.filter { CharacterSet.letters.contains($0) }
    // Pure punctuation (no letters, no digits) → keep for sentence structure
    if letters.isEmpty && !token.contains(where: { $0.isNumber }) { return true }
    // Noise tokens (standalone numbers/codes) → remove
    if isNoiseToken(token) { return false }
    let tokenScript = dominantScript(of: token)
    // Mixed tokens like "DNA가" — if token has ANY letters matching target, keep it
    if tokenScript == .other { return true }
    return scriptsCompatible(tokenScript, target)
  }

  /// Strip characters from a different script off the edges of a token.
  /// e.g. "God이" with target .latin → "God", "DNA가" with target .latin → "DNA"
  private static func cleanMixedToken(_ token: String, target: DominantScript) -> String {
    let tokenScript = dominantScript(of: token)
    // If already pure target or pure other, no cleaning needed
    if tokenScript == target { return token }

    // Check if it's actually mixed (has some target script chars)
    let hasTarget: Bool
    switch target {
    case .latin:
      hasTarget = token.unicodeScalars.contains { v in
        (0x0041...0x007A).contains(v.value) || (0x00C0...0x024F).contains(v.value)
      }
    case .hangul:
      hasTarget = token.unicodeScalars.contains { v in
        (0xAC00...0xD7AF).contains(v.value) || (0x1100...0x11FF).contains(v.value)
      }
    case .cjk, .japanese:
      hasTarget = token.unicodeScalars.contains { v in
        (0x4E00...0x9FFF).contains(v.value) || (0x3040...0x30FF).contains(v.value)
      }
    case .other:
      return token
    }
    guard hasTarget else { return token }

    // Strip non-target script characters from edges
    var result = token
    // Trim trailing non-target characters (e.g. "God이" → "God")
    while let last = result.last {
      let lastScript = dominantScript(of: String(last))
      if lastScript != target && lastScript != .other && CharacterSet.letters.contains(last.unicodeScalars.first!) {
        result = String(result.dropLast())
      } else { break }
    }
    // Trim leading non-target characters
    while let first = result.first {
      let firstScript = dominantScript(of: String(first))
      if firstScript != target && firstScript != .other && CharacterSet.letters.contains(first.unicodeScalars.first!) {
  ...
  static func filterSentenceByScript(sentence: String, selectedWord: String) -> String {
    let wordScript = dominantScript(of: selectedWord)
    guard wordScript != .other else { return sentence }
    let tokens = sentence.split(whereSeparator: { $0.isWhitespace }).map(String.init)
    let filtered = tokens.compactMap { token -> String? in
      guard tokenMatchesScript(token, wordScript) else { return nil }
      let cleaned = cleanMixedToken(token, target: wordScript)
      return cleaned.isEmpty ? nil : cleaned
    }
    let result = filtered.joined(separator: " ").trimmingCharacters(in: .whitespacesAndNewlines)
    // If filtering removed too much (>80%), return original — likely a mixed-language paragraph
    if !sentence.isEmpty && Double(result.count) / Double(sentence.count) < 0.2 {
      return sentence
    }
    return result
  }

  // MARK: - Rect-Anchored Sentence Extraction Helpers

  /// Finds the position of a line text fragment within the full page text.
  /// Tries progressively shorter word sequences for robustness against whitespace differences.
  private static func findLineAnchorIndex(
    lineText: String,
    in pageText: String
  ) -> String.Index? {
    let lineWords = lineText.split(whereSeparator: { $0.isWhitespace })
    guard !lineWords.isEmpty else { return nil }

    // Try progressively shorter fragments until we find a match
    for wordCount in stride(from: min(lineWords.count, 6), through: 2, by: -1) {
      let fragment = lineWords.prefix(wordCount).joined(separator: " ")
      if let range = pageText.range(
        of: fragment,
    ...
    // Last resort: find a unique-enough word from the line (4+ chars, appears once)
    for lineWord in lineWords where lineWord.count >= 4 {
      let wordStr = String(lineWord)
      var count = 0
      ...
      while let range = pageText.range(
        ...
        range: searchStart..<pageText.endIndex
      ) {
        count += 1
        if count == 1 { foundRange = range }
        if count > 1 { break }
        searchStart = range.upperBound
      }
      if count == 1, let range = foundRange {
        return range.lowerBound
      }
    }
  ...
  /// Finds the occurrence of `word` in `text` that is closest to `anchor` index.
  private static func findNearestWordOccurrence(
    word: String,
    near anchor: String.Index,
    in text: String
  ) -> String.Index? {
    let anchorOffset = text.distance(from: text.startIndex, to: anchor)
    ...
    while let range = text.range(
      ...
      let distance = abs(offset - anchorOffset)
      if distance < bestDistance {
        bestDistance = distance
  ...
  /// Extracts the paragraph text surrounding a given index, bounded by double newlines.
  private static func extractParagraphContext(
    around index: String.Index,
    in text: String
  ) -> (text: String, startIndex: String.Index) {
    // Expand backward to paragraph start (double newline or start of text)
    var start = index
    ...
    while start > text.startIndex {
      ...
      } else if !text[prev].isWhitespace {
        consecutiveNewlines = 0
      }
      start = prev
    }

    // Expand forward to paragraph end
    var end = index
    consecutiveNewlines = 0
    ...
    return (paragraphText, start)
  }

  /// Primary sentence extraction using PDFKit's selection(for:) to anchor to the
  /// exact visual position of the tapped word. Falls back to nil if PDFKit selection
  /// is unavailable (e.g., scanned PDFs).
  private static func extractSentenceUsingRectAnchor(
    pageText: String,
    selectedWord: String,
    rectOnPage: CGRect,
    page: PDFPage
  ) -> String? {
    // 1. Build a full-width rect at the tapped word's Y position
    let pageBounds = page.bounds(for: .mediaBox)
    let lineRect = CGRect(
      x: pageBounds.minX,
      y: rectOnPage.minY - 2,
      width: pageBounds.width,
      height: rectOnPage.height + 4
    )

    // 2. Get the line text using PDFKit
    guard let lineSelection = page.selection(for: lineRect),
          let rawLineText = lineSelection.string,
          !rawLineText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    else { return nil }
    ...
    // 3. Clean page text the same way as the fallback path
    ...
    // 4. Find the line text in full page text to get exact character position
    guard let anchorIndex = findLineAnchorIndex(
      lineText: lineText,
      in: cleanedPageText
    ) else {
      #if DEBUG
      print("[SentenceExtract] RECT-ANCHOR: line text not found in page text, falling back")
      #endif
      return nil
    }

    // 5. Find the nearest occurrence of the selected word to this anchor
    guard let wordIndex = findNearestWordOccurrence(
      word: selectedWord,
      near: anchorIndex,
      in: cleanedPageText
    ) else {
      #if DEBUG
      print("[SentenceExtract] RECT-ANCHOR: word not found near anchor, falling back")
      #endif
      return nil
    }

    // 6. Extract paragraph context around the word
    let context = extractParagraphContext(
      around: wordIndex,
      in: cleanedPageText
    )

    // 7. Use NLTokenizer to find the exact sentence
    let tokenizer = NLTokenizer(unit: .sentence)
    tokenizer.string = context.text

    let wordOffsetInFullText = cleanedPageText.distance(
      from: cleanedPageText.startIndex,
      to: wordIndex
    )
    let contextStartOffset = cleanedPageText.distance(
      from: cleanedPageText.startIndex,
      to: context.startIndex
    )
    let wordOffsetInContext = max(0, wordOffsetInFullText - contextStartOffset)
    let clampedOffset = min(wordOffsetInContext, context.text.count - 1)
    guard clampedOffset >= 0 else { return nil }

    let contextWordIndex = context.text.index(
      context.text.startIndex,
      offsetBy: clampedOffset
    )
    let sentenceRange = tokenizer.tokenRange(at: contextWordIndex)
    var sentence = String(context.text[sentenceRange])
      .trimmingCharacters(in: .whitespacesAndNewlines)
    sentence = sentence.split(whereSeparator: { $0.isWhitespace })
      .joined(separator: " ")

    guard !sentence.isEmpty else { return nil }

    // 8. Apply script filtering + word-count limits (same as existing code)
    sentence = filterSentenceByScript(sentence: sentence, selectedWord: selectedWord)
    guard !sentence.isEmpty else { return nil }

    let maxWords = 35
    let tokens = sentence.split(whereSeparator: { $0.isWhitespace }).map(String.init)
    if tokens.count > maxWords {
      let wordLow = selectedWord.lowercased()
      let anchor = tokens.firstIndex(where: { $0.lowercased().contains(wordLow) }) ?? tokens.count / 2
      let half = maxWords / 2
      let start = max(0, anchor - half)
      let end = min(tokens.count, start + maxWords)
      sentence = tokens[max(0, end - maxWords)..<end].joined(separator: " ")
    }

    if sentence.count > ReaderLookupLimits.maxPopupCharacterCount {
      sentence = String(sentence.prefix(ReaderLookupLimits.maxPopupCharacterCount))
        .trimmingCharacters(in: .whitespacesAndNewlines)
    }

    #if DEBUG
    print("[SentenceExtract] RECT-ANCHORED len=\(sentence.count)")
    #endif
    return sentence
  }

  /// Extract the complete sentence containing `selectedWord` from the full page text.
  /// Uses PDFKit rect-anchored extraction as primary method, falls back to
  /// NLTokenizer(.sentence) with position hint for boundary detection.
  /// - positionHint: 0.0 = top of page, 1.0 = bottom. Used to pick the closest match
  ///   when the word appears multiple times on the page.
  static func extractSentenceAroundWord(
    pageText: String?,
    selectedWord: String,
    positionHint: CGFloat? = nil,
    rectOnPage: CGRect? = nil,
    page: PDFPage? = nil
  ) -> String? {
    guard let pageText, !pageText.isEmpty else { return nil }
    let trimmedWord = selectedWord.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmedWord.isEmpty else { return nil }

    // --- Step 0: Rect-anchored extraction (primary, most accurate) ---
    // Uses PDFKit selection at the tapped Y position for ground-truth mapping.
    if let rectOnPage, let page {
      if let result = extractSentenceUsingRectAnchor(
        pageText: pageText,
      ...
      // Rect-anchored extraction failed — fall through to character-offset method
    }

    // --- Step 1: Split page text into lines, then group into paragraphs ---
    // Each \n in page.string = one PDF text line.
    // Short lines (≤ 50 chars) = headers/titles/labels → own paragraph.
    // Consecutive long lines (> 50 chars) = body text → merged paragraph.
    ...
    let shortLineThreshold = 50
    let allLines = cleanedText.components(separatedBy: "\n")
    var paragraphs: [(text: String, position: CGFloat)] = []
    var charOffset = 0
    let totalLen = max(1, cleanedText.count)
    ...
    func flushBody() {
      ...
      if !joined.isEmpty {
        paragraphs.append((joined, CGFloat(bodyStart) / CGFloat(totalLen)))
      }
      bodyLines = []
    }
    ...
    for line in allLines {
      ...
      defer { charOffset += line.count + 1 }
      guard !trimmed.isEmpty else { flushBody(); continue }

      if trimmed.count <= shortLineThreshold {
        flushBody()
        // Short line → its own paragraph
        paragraphs.append((trimmed, CGFloat(charOffset) / CGFloat(totalLen)))
      } else {
        // Long line → accumulate into body paragraph
        if bodyLines.isEmpty { bodyStart = charOffset }
    ...
    // --- Step 2: Pick the paragraph closest to the tap position ---
    // Always use positionHint when available — this is the most reliable signal.
    let bestIdx: Int
    if let hint = positionHint {
      // Among paragraphs that contain the word, pick closest to hint.
      // If none contain the word (smart quotes, OCR), pick closest overall.
      let matchingIndices = paragraphs.indices.filter {
        paragraphs[$0].text.range(of: trimmedWord, options: [.caseInsensitive, .diacriticInsensitive]) != nil
      }
      let candidates = matchingIndices.isEmpty ? Array(paragraphs.indices) : matchingIndices
      bestIdx = candidates.min(by: {
        abs(paragraphs[$0].position - hint) < abs(paragraphs[$1].position - hint)
      }) ?? 0
    } else {
      // No position hint — find first paragraph containing the word
      bestIdx = paragraphs.indices.first(where: {
        paragraphs[$0].text.range(of: trimmedWord, options: [.caseInsensitive, .diacriticInsensitive]) != nil
      }) ?? 0
    }

    #if DEBUG
    print("[SentenceExtract] paras=\(paragraphs.count) picked[\(bestIdx)] pos=\(String(format: "%.3f", paragraphs[bestIdx].position)) hint=\(positionHint.map { String(format: "%.3f", $0) } ?? "nil") textLen=\(paragraphs[bestIdx].text.count)")
    #endif

    // --- Step 3: Within paragraph, use NLTokenizer for exact sentence ---
    var contextText = paragraphs[bestIdx].text
    // If paragraph is very short, merge neighbors for context
    if contextText.split(whereSeparator: { $0.isWhitespace }).count < 5 {
      if bestIdx > 0 { contextText = paragraphs[bestIdx - 1].text + " " + contextText }
      if bestIdx < paragraphs.count - 1 { contextText = contextText + " " + paragraphs[bestIdx + 1].text }
    }

    let tokenizer = NLTokenizer(unit: .sentence)
    tokenizer.string = contextText

    var sentence: String
    if let wordRange = contextText.range(of: trimmedWord, options: [.caseInsensitive, .diacriticInsensitive]) {
      let sentenceRange = tokenizer.tokenRange(at: wordRange.lowerBound)
      sentence = String(contextText[sentenceRange]).trimmingCharacters(in: .whitespacesAndNewlines)
    } else {
      sentence = contextText
    }
    sentence = sentence.split(whereSeparator: { $0.isWhitespace }).joined(separator: " ")
    guard !sentence.isEmpty else { return nil }

    // --- Step 4: Filter by script + trim ---
    sentence = filterSentenceByScript(sentence: sentence, selectedWord: trimmedWord)
    guard !sentence.isEmpty else { return nil }

    // Safety: trim to 35 words around the word
    let maxWords = 35
    let tokens = sentence.split(whereSeparator: { $0.isWhitespace }).map(String.init)
    if tokens.count > maxWords {
      let wordLow = trimmedWord.lowercased()
      let anchor = tokens.firstIndex(where: { $0.lowercased().contains(wordLow) }) ?? tokens.count / 2
      let half = maxWords / 2
      let start = max(0, anchor - half)
      let end = min(tokens.count, start + maxWords)
    ...
    #if DEBUG
    print("[SentenceExtract] FINAL len=\(sentence.count)")
    #endif
    return sentence
  }
}
